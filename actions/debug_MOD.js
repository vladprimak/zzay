module.exports = {
    ...
    subtitle(data) {
        if (data.action === '1') return `Status: <span style='color: #883c51'>${data.status.replace('0', 'Off')
                                                                                    .replace('1', 'On')}</span>`;
        if (data.status === '0') return `Status: <span style='color: #d55170'>${data.status.replace('0', 'Off')
                                                                                    .replace('1', 'On')}</span>`;
        if (data.status === '1') return `Status: <span style='color: #24a2ae'>${data.status.replace('0', 'Off')
                                                                                    .replace('1', 'On')}</span>`;
    },
    ...
    action(cache) {
        ...
        const status = parseInt(data.status, cache)
        const action = parseInt(data.action, cache)
        if (status === 1) {
            let date = new Date();
            console.log(
                'Performed actions: ' + (cache.index + 1) + '\n' +
                'Date: ' + date
            );
            if (log.length > 0) {
                console.log(log);
